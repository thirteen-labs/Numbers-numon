...
<head>
<meta charset="UTF-8">
<title>Numera Report — ${escapeHtml(report.profileName)}</title>
<style>
...
<body>
  ...
  <div class="footer">
    Generated by Numera — Complete Offline Numerology Platform
  </div>
</body>
</html>`;

  const filename = `numera-report-${report.profileName.replace(/\s+/g, '-').toLowerCase()}.html`;
...
export async function exportReportAsCsv(report: ReportData): Promise<string> {
  ...
  const filename = `numera-report-${report.profileName.replace(/\s+/g, '-').toLowerCase()}.csv`;
  const path = `${Paths.cache}/${filename}`;
  const file = new File(path);
